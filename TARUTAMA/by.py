...
class Model:
    ...
    def calculate_transport_emission(self, jenis, bbm, jarak, frekuensi):
        kendaraan_factor = {
            "motor": 0.045,
            "mobil": 0.1
        }
        ...
        faktor_kendaraan = kendaraan_factor.get(jenis.lower(), 0.1)
        faktor_bbm = bbm_factor.get(bbm.lower(), 2.322)

        total_emisi = faktor_kendaraan * faktor_bbm * jarak * frekuensi
...
class TransportasiPage:
    def __init__(self, container, controller):
        ...
        label_jenis = tk.Label(self.frame, text="Jenis Kendaraan", fg="#0A2736", font=("Inter", 12, "bold"), anchor="w", justify="left")
        label_jenis.place(relx=0.25, rely=0.25)
        self.jenis_var = tk.StringVar()
        self.jenis_var.set("motor")
        jenis_dropdown = tk.OptionMenu(self.frame, self.jenis_var, "motor", "mobil")
        jenis_dropdown.config(width=20, font=("Inter", 10))
        jenis_dropdown.place(relx=0.5, rely=0.325, anchor="center")

        # Jenis bahan bakar (dropdown)
        label_bbm = tk.Label(self.frame, text="Jenis Bahan Bakar", fg="#0A2736", font=("Inter", 12, "bold"), anchor="w", justify="left")
        label_bbm.place(relx=0.25, rely=0.375)
        self.bbm_var = tk.StringVar()
        self.bbm_var.set("pertalite")
        bbm_dropdown = tk.OptionMenu(self.frame, self.bbm_var, "pertalite", "pertamax", "solar")
        bbm_dropdown.config(width=20, font=("Inter", 10))
        bbm_dropdown.place(relx=0.5, rely=0.45, anchor="center")

        # Jarak tempuh
        label_jarak = tk.Label(self.frame, text="Jarak Tempuh (km)", fg="#0A2736", font=("Inter", 12, "bold"), anchor="w", justify="left")
        label_jarak.place(relx=0.25, rely=0.5)
        self.entry_jarak = tk.Entry(self.frame, bg="#D9D9D9", fg="#0A2736", font=("Inter", 12))
        self.entry_jarak.place(relx=0.5, rely=0.575, anchor="center", width=200, height=30)

        # Frekuensi
        label_freq = tk.Label(self.frame, text="Frekuensi (kali/minggu)", fg="#0A2736", font=("Inter", 12, "bold"), anchor="w", justify="left")
        label_freq.place(relx=0.25, rely=0.625)
        self.entry_freq = tk.Entry(self.frame, bg="#D9D9D9", fg="#0A2736", font=("Inter", 12))
        self.entry_freq.place(relx=0.5, rely=0.7, anchor="center", width=200, height=30)
        ...
        hitung_btn.place(relx=0.5, rely=0.8, anchor="center")
        ...
        kembali_btn.place(relx=0.5, rely=0.9, anchor="center")

    def handle_calculation(self):
        jenis = self.jenis_var.get()
        bbm = self.bbm_var.get()
        jarak = self.entry_jarak.get()
        frekuensi = self.entry_freq.get()

        if not jenis or not bbm or not jarak or not frekuensi:
        ...
        try:
            ...
            self.controller.calculate_transport_emission(jenis, bbm, jarak, frekuensi)
        except ValueError:
...
class Controller:
    ...
    def calculate_transport_emission(self, jenis, bbm, jarak, frekuensi):
        try:
            jarak = float(jarak)
            frekuensi = int(frekuensi)
            data = self.model.calculate_transport_emission(jenis, bbm, jarak, frekuensi)
